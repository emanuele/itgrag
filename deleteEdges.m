...
% retVal3: deltas
%
function [coordinates, adjacencyList, deltas] = deleteEdges(coord, adjL, parameters)
  % check parameters
  ...
  for i=1:n
    ...
    if(!length(find(adjacencyList{sourceNode} == targetNode)))
      error("Edge %d %d does not exist.", sourceNode, targetNode);
    endif
    % delete edge
    adjEntries=adjacencyList{sourceNode};
    adjEntries(find(adjEntries==targetNode))=[];
    adjacencyList{sourceNode}=adjEntries;
    adjEntries=adjacencyList{targetNode};
    adjEntries(find(adjEntries==sourceNode))=[];
    adjacencyList{targetNode}=adjEntries; 
  endfor
  
  deltas = {"edge", "delete", edges};
